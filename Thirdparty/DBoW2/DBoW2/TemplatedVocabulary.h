...
public:
     ...
     */
    virtual void
    transform(const std::vector<TDescriptor> &features, BowVector &v) const;
    ...
    virtual void transform(
        const std::vector<TDescriptor> &features, BowVector &v,
...
protected:
     ...
     */
    virtual void transform(
        const TDescriptor &feature, WordId &id, WordValue &weight,
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::transform(
    const TDescriptor &feature, WordId &word_id, WordValue &weight, NodeId *nid,
    int levelsup) const {
...
template <class TDescriptor, class F>
void TemplatedVocabulary<TDescriptor, F>::save(
    cv::FileStorage &f, const std::string &name) const {
    // Format YAML:
    // vocabulary
    // {
    //   k:
...
void TemplatedVocabulary<TDescriptor, F>::load(
    const cv::FileStorage &fs, const std::string &name) {
    m_words.clear();
    m_nodes.clear();
...
template <class TDescriptor, class F>
std::ostream &
operator<<(std::ostream &os, const TemplatedVocabulary<TDescriptor, F> &voc) {
