...
// --------------------------------------------------------------------------
    ...
    {
        std::vector<NodeId> vNodeIds;
        transform(features, v, fv, vNodeIds, levelsup);
    }
...
// --------------------------------------------------------------------------
    ...
    bool Vocabulary::load(std::istream &ifile)
    {
        ...
        if (sig != 20180817000) // Check if it is a binary file.
            return false;
    ...
    int Vocabulary::LoadFromMemory(const char* mem, const unsigned int m_size)
    {
        ...
        if(nnodes==0 || nnodes != 709952){
//            LOGD("LoadFromMemory: invalid voc. nodes.size = %d.\n", nnodes);
            return -2;
        }
//        LOGD("LoadFromMemory start, nnodes = %d\n", nnodes);
...
//        LOGD("LoadFromMemory compressed, time %lf\n", timer.Duration());
...
//        LOGD("LoadFromMemory read 1, time %lf\n", timer.Duration());
        createScoringObject();

//        LOGD("LoadFromMemory read 2, time %lf\n", timer.Duration());
...
//        LOGD("LoadFromMemory read 3, time %lf\n", timer.Duration());
        //    // words
...
//        LOGD("LoadFromMemory read 4, time %lf\n", timer.Duration());
        return 0;
    }
    ...
    void Vocabulary::toStream(  std::ostream &out_str, bool compressed) const {
        ...
        uint64_t sig=20180817000;//magic number describing the file
    ...
    void Vocabulary::fromStream(std::istream &str) {
        ...
        uint64_t sig=0;//magic number describing the file
        str.read((char*)&sig,sizeof(sig));
        if (sig!=20180817000) throw std::runtime_error("Vocabulary::fromStream  is not of appropriate type");
        bool compressed;
        str.read((char*)&compressed,sizeof(compressed));
        uint32_t nnodes;
        str.read((char*)&nnodes,sizeof(nnodes));
