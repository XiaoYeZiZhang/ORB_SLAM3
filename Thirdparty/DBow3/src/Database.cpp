...
Database::Database(const std::shared_ptr<Vocabulary> pVoc, bool use_di, int di_levels)
  ...
  (const Database &db)
{
  if(this != &db)
  {
    ...
    if (db.m_voc) SetVocabulary(db.m_voc);
  }
  return *this;
}
...
{
  std::lock_guard<std::mutex> lck(mQueryMutex);
  ...
  if(m_use_di)
  {
    ...
    if(entry_id == m_dfile.size())
    {
      ...
      m_dfile_valid.push_back(true);
    }
    else
    {
      m_dfile[entry_id] = fv;
      m_dfile_valid[entry_id] = true;
    }
  }
...
void Database::remove(const DBoW3::EntryId &entryId) {
//  for (int i = 0; i < m_ifile.size(); ++i) {
//      IFRow& row = m_ifile[i];
//    for (auto rit = row.begin(); rit != row.end(); ++rit) {
//      if (rit->entry_id == entryId){
//        row.erase(rit);
//      }
//      else if(rit->entry_id > entryId){
//        rit->entry_id = rit->entry_id - 1;
//      }
//    }
//  }
//  --m_nentries;
//  m_dfile.erase(m_dfile.begin() + entryId);
  std::lock_guard<std::mutex> lck(mQueryMutex);
  m_dfile_valid[entryId] = false;
}
...
// --------------------------------------------------------------------------
  ...
  (std::shared_ptr<Vocabulary> pVoc)
{
  m_voc = pVoc;
  clear();
}
...
// --------------------------------------------------------------------------
  ...
  void Database::setVocabulary
  (std::shared_ptr<Vocabulary> voc, bool use_di, int di_levels)
{
  m_use_di = use_di;
...
{
  std::vector<bool> inlier_mask(m_dfile_valid.size(), true);
  query(vec, inlier_mask, ret, max_results, max_id);
}
  ...
  QueryResults &ret, int max_results, int max_id)
{
  ...
  {
    case L1_NORM:
      queryL1(vec, inlier_mask, ret, max_results, max_id);
      break;
        ...
        QueryResults &ret, int max_results, int max_id)
{
  ...
  for(vit = vec.begin(); vit != vec.end(); ++vit)
  {
    ...
    for(auto rit = row.begin(); rit != row.end(); ++rit)
    {
      ...
      if (entry_id >= m_dfile_valid.size())
        continue;
      if(((int)entry_id < max_id || max_id == -1)
        && (m_dfile_valid[entry_id])
        && (entry_id < inlier_mask.size() && inlier_mask[entry_id]))
      {
  ...
  const std::string &name)
{
  ...
  if(!m_voc) m_voc = std::make_shared<Vocabulary>();
