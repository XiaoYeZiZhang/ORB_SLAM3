...
//    int di_levels = 0);
  explicit Database(const std::shared_ptr<Vocabulary> pVoc, bool use_di = true,
          int di_levels = 0);
...
//  void setVocabulary(const Vocabulary &voc);
  ...
  void setVocabulary(std::shared_ptr<Vocabulary> voc, bool use_di, int di_levels = 0);
   ...
   *   < 0 means all
   */
  void query(const BowVector &vec,
...
protected:
  ...
  void queryL1(const BowVector &vec, const std::vector<bool>& inlier_mask,
