...
public:
    ...
    //    int di_levels = 0);
    explicit Database(
        const std::shared_ptr<Vocabulary> pVoc, bool use_di = true,
        int di_levels = 0);
    ...
    void SetVocabulary(std::shared_ptr<Vocabulary> pVoc);
    ...
    void setVocabulary(
        std::shared_ptr<Vocabulary> voc, bool use_di, int di_levels = 0);

    /**
     * Returns a pointer to the vocabulary used
     * @return vocabulary
     */
    const std::shared_ptr<Vocabulary> getVocabulary() const;
     ...
     */
    EntryId
    add(const std::vector<cv::Mat> &features, BowVector *bowvec = NULL,
     ...
     */
    EntryId
    add(const cv::Mat &features, BowVector *bowvec = NULL,
        FeatureVector *fvec = NULL);
     ...
     */
    EntryId
    add(const BowVector &vec, const FeatureVector &fec = FeatureVector());

    void remove(const EntryId &entryId);
     ...
     */
    void query(
        const std::vector<cv::Mat> &features, QueryResults &ret,
     ...
     */
    void query(
        const cv::Mat &features, QueryResults &ret, int max_results = 1,
        int max_id = -1);
    /**
     * Queries the database with some features
     * @param features query features, one per row
     * @param inlier_mask search inlier image
     * @param ret (out) query results
     * @param max_results number of results to return. <= 0 means all
     * @param max_id only entries with id <= max_id are returned in ret.
     *   < 0 means all
     */
    void query(
        const BowVector &vec, const std::vector<bool> &inlier_mask,
        QueryResults &ret, int max_results = 1, int max_id = -1);
     ...
     */
    void query(
        const BowVector &vec, QueryResults &ret, int max_results = 1,
        int max_id = -1);
     ...
     */
    virtual void
    save(cv::FileStorage &fs, const std::string &name = "database") const;
     ...
     */
    virtual void
    load(const cv::FileStorage &fs, const std::string &name = "database");
...
protected:
    ...
    void queryL2(
        const BowVector &vec, QueryResults &ret, int max_results,
        int max_id) const;
...
protected:
    ...
    std::shared_ptr<Vocabulary> m_voc;
