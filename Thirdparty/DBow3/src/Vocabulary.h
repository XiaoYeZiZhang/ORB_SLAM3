...
public:
     ...
     */
    Vocabulary(
        int k = 10, int L = 5, WeightingType weighting = TF_IDF,
     ...
     */
    virtual void
    transform(const std::vector<cv::Mat> &features, BowVector &v) const;
    /**
     * Transforms a set of descriptores into a bow vector
    ...
    virtual void transform(
        const std::vector<cv::Mat> &features, BowVector &v, FeatureVector &fv,
        int levelsup) const;

    virtual void transform(
        const std::vector<cv::Mat> &features, BowVector &v, FeatureVector &fv,
        std::vector<NodeId> &vNodeIds, int levelsup) const;
     ...
     */
    virtual void
    save(cv::FileStorage &fs, const std::string &name = "vocabulary") const;
     ...
     */
    virtual void
    load(const cv::FileStorage &fs, const std::string &name = "vocabulary");
    ...
    int LoadFromMemory(const char *mem, const unsigned int m_size);
...
protected:
     ...
     */
    virtual void transform(
        const cv::Mat &feature, WordId &id, WordValue &weight, NodeId *nid,
     ...
     */
    virtual void
    transform(const cv::Mat &feature, WordId &id, WordValue &weight) const;
     ...
     */
    void HKmeansStep(
        NodeId parent_id, const std::vector<cv::Mat> &descriptors,
        int current_level);
    ...
    operator<<(std::ostream &os, const Vocabulary &voc);
