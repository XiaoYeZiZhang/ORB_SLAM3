...
struct ScopeTimer {
    ...
    ~ScopeTimer() {
        if (use) {
            ...
            // std::cout << "Time ("<<name<<")=
            // "<<double(std::chrono::duration_cast<std::chrono::nanoseconds>(end-begin).count())/fact<<str<<std::endl;
            // ;
        }
    }
};
...
struct ScopedTimerEvents {
    ...
    ScopedTimerEvents(
        std::string name = "", bool start = true, SCALE _sc = MSEC) {
        if (start)
            add("start");
        sc = _sc;
        _name = name;
...
struct Timer {
    ...
    void end() {
        ...
        sum +=
            double(std::chrono::duration_cast<std::chrono::nanoseconds>(e - _s)
                       .count());
        n++;
    }
