...
void TestViewer::LoadImages(const string &strImagePath, const string &strPathTimes,
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ifstream fTimes;
...
void TestViewer::LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ifstream fImu;
...
ORB_SLAM3::System* TestViewer::InitializeSLAM(int argc, char* argv[]) {
    ...
    ORB_SLAM3::System* SLAM = new ORB_SLAM3::System(argv[1],argv[2],ORB_SLAM3::System::IMU_MONOCULAR, true);
    return SLAM;
