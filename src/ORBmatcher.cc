                        ...
                        if (F.Nleft != -1 &&
                            F.mvLeftToRightMatch[bestIdx] !=
                                -1) { // Also match with the stereo observation
                                      // at right camera
                            F.mvpMapPoints
                                [F.mvLeftToRightMatch[bestIdx] + F.Nleft] = pMP;
                            nmatches++;
                            right++;
                        }
...
bool ORBmatcher::CheckDistEpipolarLine2(
    const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &F12,
...
int ORBmatcher::Fuse(
    KeyFrame *pKF, const vector<MapPoint *> &vpMapPoints, const float th,
    const bool bRight) {
    cv::Mat Rcw, tcw, Ow;
...
int ORBmatcher::Fuse(
    KeyFrame *pKF, cv::Mat Scw, const vector<MapPoint *> &vpPoints, float th,
...
int ORBmatcher::SearchByProjection(
    Frame &CurrentFrame, KeyFrame *pKF, const set<MapPoint *> &sAlreadyFound,
