...
public:
    ...
    void DivideNode(
        ExtractorNode &n1, ExtractorNode &n2, ExtractorNode &n3,
        ExtractorNode &n4);
...
vector<cv::KeyPoint> DistributeOctTree(
        ...
        if ((int)lNodes.size() >= N || (int)lNodes.size() == prevSize) {
            ...
            while (!bFinish) {
                ...
                sort(
                    ...
                    [](const pair<int, ExtractorNode *> &p1,
                       const pair<int, ExtractorNode *> &p2) {
                        if (p1.first < p2.first)
                            return true;
                        else if (p1.first == p2.first) {
                            if (p1.second->BL.x < p2.second->BL.x)
                                return true;
                            else if (p1.second->BL.x == p2.second->BL.x)
                                return p1.second->BL.y < p2.second->BL.y;
                        } else
                            return false;
                        return false;
                    });
...
int ComputeKeyPointsOrientation(
    ...
    if ((image.empty()) || (CV_8UC1 != image.type()) ||
        (umax.size() <= HALF_PATCH_SIZE)) {
        return -1;
    }
...
void ComputeOrbDescriptor(
    ...
    float angle = (float)kpt.angle * (ORBExtractor::factorPI);
...
int ORBExtractor::ComputePyramid(
    const cv::Mat &image, std::vector<cv::Mat> &outImagePyramid) {
    if ((image.empty()) || (CV_8UC1 != image.type())) {
        return -1;
    }

    outImagePyramid.resize(nlevels);
    for (int level = 0; level < nlevels; ++level) {
        ...
        if (level != 0) {
            resize(
                outImagePyramid[level - 1], outImagePyramid[level], sz, 0, 0,
                INTER_LINEAR);

            copyMakeBorder(
                outImagePyramid[level], temp, EDGE_THRESHOLD, EDGE_THRESHOLD,
...
int ORBExtractor::DetectRawFastKeyPoints(
...
const float ORBExtractor::factorPI = (float)(CV_PI / 180.f);

static inline int GetKeyPointsCount(
    const std::vector<std::vector<cv::KeyPoint>> &keyPointsWithScale) {
    int result = 0;
    for (const auto &kpts : keyPointsWithScale) {
        result += kpts.size();
    }
    return result;
}
...
ORBExtractor::ORBExtractor(
    int _nfeatures, float _scaleFactor, int _nlevels, int _iniThFAST,
    int _minThFAST)
    : nfeatures(_nfeatures), nlevels(_nlevels), iniThFAST(_iniThFAST),
      minThFAST(_minThFAST), m_imagePyramidKey(nullptr) {
    ...
    static_assert(
        sizeof(bit_pattern_31_) == (sizeof(Point) * kPatternSize),
        "pattern size error");
    const Point *pattern0 = (const Point *)bit_pattern_31_;
    std::copy(pattern0, pattern0 + kPatternSize, std::back_inserter(pattern));
...
int ORBExtractor::DetectKeyPoints(
    const cv::Mat &image, std::vector<cv::Mat> &outImagePyramid,
    std::vector<std::vector<cv::KeyPoint>> &outKeyPointsWithScale,
    const cv::Mat &mask) {
    std::vector<cv::Mat> imagePyramid;
    int result = ComputePyramid(image, imagePyramid);
    if (0 != result) {
        return result;
    }

    std::vector<std::vector<cv::KeyPoint>> keyPointsWithScale;
    result = DetectRawFastKeyPoints(imagePyramid, keyPointsWithScale);
    if (0 != result) {
        return result;
    }

    result = PostProcessKeyPoints(imagePyramid, keyPointsWithScale, mask);
    if (0 != result) {
        return result;
    }

    outImagePyramid = std::move(imagePyramid);
    outKeyPointsWithScale = std::move(keyPointsWithScale);
    return result;
}
...
int ORBExtractor::DetectKeyPoints(
    ...
    SetImagePyramidCache(image, imagePyramid);
    outKeyPoints = ScaleKeyPointsToRawImage(keyPointsWithScale);
    return result;
}

int ORBExtractor::ComputeDescriptors(
    const std::vector<cv::Mat> &imagePyramid,
    std::vector<std::vector<cv::KeyPoint>> &keyPointsWithScale,
    cv::Mat &outDescriptors, const cv::Mat *imagePyramidGaussData,
    int imagePyramidGaussCount) {
    bool isNeedGaussianBlur =
        ((nullptr == imagePyramidGaussData) || (imagePyramidGaussCount <= 0));
    if ((nlevels != imagePyramid.size()) ||
        (nlevels != keyPointsWithScale.size()) ||
        ((!isNeedGaussianBlur) && (nlevels != imagePyramidGaussCount))) {
        return -1;
    }

    int nkeypoints = GetKeyPointsCount(keyPointsWithScale);
    if (nkeypoints <= 0) {
        return -1;
    }

    outDescriptors.create(nkeypoints, 32, CV_8UC1);

    int offset = 0;
    int result = 0;
    for (int level = 0; (level < nlevels) && (0 == result); ++level) {
        vector<KeyPoint> &keypoints = keyPointsWithScale[level];
        ...
        Mat workingMat;
        if (!isNeedGaussianBlur) {
            workingMat = imagePyramidGaussData[level];
        } else {
            workingMat = imagePyramid[level].clone();
            GaussianBlur(
                workingMat, workingMat, Size(7, 7), 2, 2, BORDER_REFLECT_101);
        }

        // Compute the descriptors
        Mat desc = outDescriptors.rowRange(offset, offset + nkeypointsLevel);
        result = ComputeDescriptorsWithoutScale(workingMat, keypoints, desc);
...
int ORBExtractor::ComputeDescriptors(
    ...
    std::vector<std::vector<cv::KeyPoint>> keyPointsWithScale =
        ScaleKeyPointsToImagePyramid(keyPoints);
    if (keyPointsWithScale.empty()) {
        return -1;
    }

    int result = 0;
    std::vector<cv::Mat> imagePyramid = ImagePyramidCache(image);
    if (imagePyramid.empty()) {
        result = ComputePyramid(image, imagePyramid);
        if (0 != result) {
            return result;
        }
    }
    ...
    keyPoints = ScaleKeyPointsToRawImage(keyPointsWithScale);
    return result;
}
...
int ORBExtractor::CullKeyPoints(
    ...
    const cv::Mat &mask) {
    if (nlevels != inOutKeyPoints.size()) {
        return -1;
    }
...
int ORBExtractor::PostProcessKeyPoints(
    const std::vector<cv::Mat> &imagePyramid,
    std::vector<std::vector<cv::KeyPoint>> &inOutKeyPoints,
    const cv::Mat &mask) {
    int result = CullKeyPoints(imagePyramid, inOutKeyPoints, mask);
    if (0 != result) {
        return result;
    }

    for (int level = 0; (0 == result) && (level < nlevels); ++level) {
        result = ComputeKeyPointsOrientation(
            imagePyramid[level], inOutKeyPoints[level], umax);
    }

    return result;
}

std::vector<cv::KeyPoint> ORBExtractor::ScaleKeyPointsToRawImage(
    const std::vector<std::vector<cv::KeyPoint>> &keyPointsWithScale) {
    ...
    result.insert(
        result.end(), keyPointsWithScale.front().cbegin(),
        keyPointsWithScale.front().cend());
    for (int level = 1; level < nlevels; ++level) {
        // Scale keypoint coordinates
        for (const KeyPoint &keypoint : keyPointsWithScale[level]) {
            cv::KeyPoint normal_kpt = keypoint;
            normal_kpt.pt *= mvScaleFactor[level];
            result.emplace_back(std::move(normal_kpt));
        }
    }
    return result;
}
...
ORBExtractor::ScaleKeyPointsToImagePyramid(
    const std::vector<cv::KeyPoint> &keyPoints) {
    std::vector<std::vector<cv::KeyPoint>> result;
    if (keyPoints.empty()) {
        return result;
    }
    ...
    for (const auto &kpt : keyPoints) {
        cv::KeyPoint small_kpt = kpt;
        // for octave out of bound, force set to octave=0
        int octave = kpt.octave;
        if (octave < 0 || octave > nlevels - 1) {
            octave = 0;
        }
        // rescale to small size
        small_kpt.pt *= mvInvScaleFactor[octave];
        result[octave].emplace_back(std::move(small_kpt));
    }
...
std::vector<cv::Mat>
ORBExtractor::ImagePyramidCache(const cv::Mat &keyImage) const {
    if ((!keyImage.empty()) && (!m_imagePyramidCache.empty()) &&
        (m_imagePyramidKey == keyImage.data) &&
        (m_imagePyramidCache.front().size() == keyImage.size())) {
        return m_imagePyramidCache;
    }

    return std::vector<cv::Mat>();
}

void ORBExtractor::SetImagePyramidCache(
    const cv::Mat &keyImage, const std::vector<cv::Mat> &imagePyramid) {
    if (keyImage.empty() || (nlevels != imagePyramid.size())) {
        return;
    }

    m_imagePyramidKey = keyImage.data;
    m_imagePyramidCache = imagePyramid;
}
} // namespace SLAMCommon