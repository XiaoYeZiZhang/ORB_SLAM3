...
public:
    ...
    void set_trans(Eigen::Matrix3d rot, Eigen::Vector3d offset) {
        //        printf("Warning, only cal Position, ignoring normal
    ...
    void Draw() {
        ...
        //                number = std::to_string(normalNr++); // transfer
        //                unsigned int to stream
        //            else if(name == "texture_height")
        //                number = std::to_string(heightNr++); // transfer
        //                unsigned int to stream
        //
        ...
        if (textures.size() > 0) {
            ...
            glActiveTexture(
                GL_TEXTURE0 + 0); // active proper texture unit before binding
...
private:
    ...
    //        array. glBufferData(GL_ARRAY_BUFFER, vertices.size() *
