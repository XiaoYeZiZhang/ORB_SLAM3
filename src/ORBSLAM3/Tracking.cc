...
Tracking::Tracking(
    ...
    R_l.empty() || R_r.empty() || D_l.empty() || D_r.empty()
        ...
        else
        {
            ...
            // M1r y M2r son las matrices relativas al mapeo correspondiente a
    la rectificación de mapa en el eje X e Y respectivamente
    ...
    f_track_times
        << "# ORB_Ext(ms), Stereo matching(ms), Preintegrate_IMU(ms), Pose "
           "pred(ms), LocalMap_track(ms), NewKF_dec(ms)"
        << endl;
    f_track_times << fixed;
#endif
}
...
cv::Mat Tracking::GrabImageStereo(
    ...
    mCurrentFrame.imgLeft = mImGray.clone();
    mCurrentFrame.imgRight = mImRight.clone();
...
cv::Mat Tracking::GrabImageMonocular(
    ...
    if (mSensor == System::MONOCULAR) {
        if (mState == NOT_INITIALIZED || mState == NO_IMAGES_YET ||
            ...
            mCurrentFrame = Frame(
                mImGray, timestamp, mpIniORBextractor, mpORBVocabulary,
                mpCamera, mDistCoef, mbf, mThDepth);
        else
            mCurrentFrame = Frame(
                mImGray, timestamp, mpORBextractorLeft, mpORBVocabulary,
                mpCamera, mDistCoef, mbf, mThDepth);
    ...
    mCurrentFrame.mnDataset = mnNumDataset;
    // for objectRecognition
    mCurrentFrame.imgLeft = im.clone();
...
void Tracking::PreintegrateIMU() {
    ...
    Verbose::PrintMess(
        "Preintegration is finished!! ", Verbose::VERBOSITY_DEBUG);
}
...
void Tracking::ComputeGyroBias(
    const vector<Frame *> &vpFs, float &bwx, float &bwy, float &bwz) {
...
void Tracking::ComputeVelocitiesAccBias(
    const vector<Frame *> &vpFs, float &bax, float &bay, float &baz) {
    const int N = vpFs.size();
...
void Tracking::Track() {
    ...
    if (mState == NOT_INITIALIZED) {
        ...
        if (!mbOnlyTracking) {
                ...
                if (mState == RECENTLY_LOST) {
                    ...
                    if ((mSensor == System::IMU_MONOCULAR ||
                        ...
                        if (mCurrentFrame.mTimeStamp - mTimeStampLost >
                            ...
                            mState = LOST;
                            Verbose::PrintMess(
                                "Track Lost...", Verbose::VERBOSITY_NORMAL);
                            bOK = false;
                        }
                    } else {
            ...
            mTime_PosePred = std::chrono::duration_cast<
            ...
            if (mState == LOST) {
                if (mSensor == System::IMU_MONOCULAR ||
                    mSensor == System::IMU_STEREO)
                    Verbose::PrintMess(
                        "IMU. State LOST", Verbose::VERBOSITY_NORMAL);
                bOK = Relocalization();
            } else {
                if (!mbVO) {
        ...
        if (bOK || mState == RECENTLY_LOST) {
            ...
            mTime_NewKF_Dec = std::chrono::duration_cast<
                                  std::chrono::duration<double, std::milli>>(
            ...
            for (int i = 0; i < mCurrentFrame.Nleft; i++) {
                ...
                if (rightIndex != -1) {
                    ...
                    pNewMP->AddObservation(
                        pKFini, rightIndex + mCurrentFrame.Nleft);

                    pKFini->AddMapPoint(pNewMP, i);
                    pKFini->AddMapPoint(
                        pNewMP, rightIndex + mCurrentFrame.Nleft);
...
void Tracking::CreateInitialMapMonocular() {
    ...
    {
        Verbose::PrintMess(
            "Wrong initialization, reseting...", Verbose::VERBOSITY_NORMAL);
        mpSystem->ResetActiveMap();
        return;
    }
...
bool Tracking::TrackWithMotionModel() {
    ...
    fill(
        mCurrentFrame.mvpMapPoints.begin(), mCurrentFrame.mvpMapPoints.end(),
        static_cast<MapPoint *>(NULL));
    ...
    if (nmatches < 20) {
        Verbose::PrintMess(
            "Not enough matches, wider window search!!",
            Verbose::VERBOSITY_NORMAL);
        fill(
...
bool Tracking::TrackLocalMap() {
    ...
    if (!mpAtlas->isImuInitialized())
        ...
        if (mCurrentFrame.mnId <= mnLastRelocFrameId + mnFramesToResetIMU) {
            Verbose::PrintMess(
                "TLM: PoseOptimization ", Verbose::VERBOSITY_DEBUG);
            Optimizer::PoseOptimization(&mCurrentFrame);
        } else {
...
void Tracking::CreateNewKeyFrame() {
        ...
        mSensor != System::IMU_MONOCULAR) // TODO check if incluide imu_stereo
    {
        ...
        if (!vDepthIdx.empty()) {
            ...
            Verbose::PrintMess(
                "new mps for stereo KF: " + to_string(nPoints),
                Verbose::VERBOSITY_NORMAL);
        }
    }
...
void Tracking::SearchLocalPoints() {
    ...
    if (nToMatch > 0) {
        ...
        if (mpAtlas->isImuInitialized()) {
            ...
            !mpAtlas->isImuInitialized() && (mSensor == System::IMU_MONOCULAR ||
                                             mSensor == System::IMU_STEREO)) {
            th = 10;
        }
...
bool Tracking::Relocalization() {
    ...
    if (vpCandidateKFs.empty()) {
        Verbose::PrintMess(
            "There are not candidates", Verbose::VERBOSITY_NORMAL);
        return false;
    }
...
void Tracking::Reset(bool bLocMap) {
    ...
    if (!bLocMap) {
        Verbose::PrintMess(
            "Reseting Local Mapper...", Verbose::VERBOSITY_NORMAL);
...
void Tracking::UpdateFrameIMU(
    const float s, const IMU::Bias &b, KeyFrame *pCurrentKeyFrame) {
