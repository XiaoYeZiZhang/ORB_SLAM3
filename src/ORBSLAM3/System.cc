...
System::System(
    ...
    endl;
        ...
        if(!isRead)
        {
            cout << "Error to load the file, please try with other session file
    or vocabulary file" << endl; exit(-1);
        }
        mpKeyFrameDatabase = new KeyFrameDatabase(*mpVocabulary);
...
void System::SaveDebugData(const int &initIdx) {
    ...
    f.open(
        "init_GDir_" + to_string(mpLocalMapper->mInitSect) + ".txt",
        ios_base::app);
    f << fixed;
    ...
    f.open(
        "init_Time_" + to_string(mpLocalMapper->mInitSect) + ".txt",
        ios_base::app);
    f << fixed;
