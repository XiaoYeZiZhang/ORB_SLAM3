...
#include "include/ORBSLAM3/Sim3Solver.h"
            ...
            if (NewDetectCommonRegions()) {
                if (mbMergeDetected) {
                    if ((mpTracker->mSensor == System::IMU_MONOCULAR) &&
                        ...
                        Verbose::PrintMess(
                            "*Merged detected", Verbose::VERBOSITY_QUIET);
                        Verbose::PrintMess(
                ...
                if (mbLoopDetected) {
                    ...
                    if (mpCurrentKF->GetMap()->IsInertial()) {
                        ...
                        // cout << "tw2w1: " << g2oSww_new.translation() <<
                        // endl; cout << "Rw2w1: " <<
                        // g2oSww_new.rotation().toRotationMatrix() << endl;
                        // cout
                        // << "Angle Rw2w1: " << 180*phi/3.14 << endl; cout <<
...
bool LoopClosing::NewDetectCommonRegions() {
    ...
    if (mnMergeNumCoincidences > 0) {
        ...
        bool bCommonRegion = DetectAndReffineSim3FromLastKF(
            ...
            vpMatchedMPs);
        if (bCommonRegion) {
            // cout << "BoW: Merge reffined Sim3 transformation suscelful" <<
            // endl;
...
bool LoopClosing::DetectAndReffineSim3FromLastKF(
    ...
    if (nNumProjMatches >= nProjMatches) {
        ...
        int numOptMatches = Optimizer::OptimizeSim3(
            mpCurrentKF, pMatchedKF, vpMatchedMPs, gScm, 10, bFixedScale,
            mHessian7x7, true);
        cout << "REFFINE-SIM3: Optimize Sim3 from last KF with "
             << numOptMatches << " inliers" << endl;
        ...
        if (numOptMatches > nProjOptMatches) {
            ...
            nNumProjMatches = FindMatchesByProjection(
                ...
                vpMPs, vpMatchedMPs);
            // cout << "REFFINE-SIM3: Projection with optimize Sim3 from last KF
            // with " << nNumProjMatches << " matches" << endl;
    ...
    for (KeyFrame *pKFi : vpBowCand) {
            ...
            numBoWMatches >= nBoWMatches) // TODO pick a good threshold
        {
            ...
            Sim3Solver solver = Sim3Solver(
                mpCurrentKF, pMostBoWMatchesKF, vpMatchedPoints, bFixedScale,
                vpKeyFrameMatchedMP);
            ...
            if (bConverge) {
                // cout <<"BoW: " << nInliers << " inliers in Sim3Solver" <<
                // endl;
                ...
                if (numProjMatches >= nProjMatches) {
                    ...
                    // optimization" << endl; cout << "Inliers in Sim3
                    // optimization: " << numOptMatches << endl;
                    ...
                    if (numOptMatches >= nSim3Inliers) {
                        ...
                        int numProjOptMatches = matcher.SearchByProjection(
                            ...
                            1.0);
                        // cout <<"BoW: " << numProjOptMatches << " matches
                        // after of the Sim3 optimization" << endl;

                        if (numProjOptMatches >= nProjOptMatches) {
                            cout << "BoW: Current KF " << mpCurrentKF->mnId
                                 << "; candidate KF " << pKFi->mnId << endl;
                            cout << "BoW: There are " << numProjOptMatches
                                 << " matches between them with the optimized "
                                    "Sim3"
                                 << endl;
                            ...
                            // cout << "BoW: Coord in X -> " << min_x << ", " <<
                            // max_x << "; and Y -> " << min_y << ", " << max_y
                            // << endl; cout << "BoW: features area in X -> " <<
                            // (max_x - min_x) << " and Y -> " << (max_y -
                            // min_y)
                            // << endl;
                            ...
                            // for(int j=0; j<vpCurrentCovKFs.size(); ++j)
                            {
                                ...
                                if (bValid) {
                                    ...
                                    // " << numProjMatches_j << " matches" <<
                                    // endl;
                                    cv::Mat Tc_w = mpCurrentKF->GetPose();
                                    cv::Mat Tw_cj = pKFj->GetPoseInverse();
                                    cv::Mat Tc_cj = Tc_w * Tw_cj;
                                    cv::Vec3d vector_dist =
                                        Tc_cj.rowRange(0, 3).col(3);
                                    cv::Mat Rc_cj =
                                        Tc_cj.rowRange(0, 3).colRange(0, 3);
                                    double dist = cv::norm(vector_dist);
                                    cout << "BoW: KF " << pKFi->mnId
                                         << " to KF " << pKFj->mnId
                                         << " is separated by " << dist
                                         << " meters" << endl;
                                    cout << "BoW: Rotation between KF -> "
                                         << Rc_cj << endl;
                                    vector<float> v_euler =
                                        Converter::toEuler(Rc_cj);
                                    v_euler[0] *= 180 / 3.1415;
                                    v_euler[1] *= 180 / 3.1415;
                                    v_euler[2] *= 180 / 3.1415;
                                    cout << "BoW: Rotation in angles (x, y, z) "
                                            "-> ("
                                         << v_euler[0] << ", " << v_euler[1]
                                         << ", " << v_euler[2] << ")" << endl;
                                    nNumKFs++;
...
bool LoopClosing::DetectCommonRegionsFromLastKF(
    ...
    std::vector<MapPoint *> &vpMatchedMPs) {
    set<MapPoint *> spAlreadyMatchedMPs(
        vpMatchedMPs.begin(), vpMatchedMPs.end());
    nNumProjMatches = FindMatchesByProjection(
        pCurrentKF, pMatchedKF, gScw, spAlreadyMatchedMPs, vpMPs, vpMatchedMPs);
    ...
    if (nNumProjMatches >= nProjMatches) {
        ...
        // "; and Y -> " << min_y << ", " << max_y << endl; cout <<
        // "PR_From_LastKF: features area in X -> " << (max_x - min_x) << " and
        // Y
        // -> " << (max_y - min_y) << endl;
...
void LoopClosing::CorrectLoop() {
...

    {
        ...
        for (vector<KeyFrame *>::iterator vit = mvpCurrentConnectedKFs.begin(),
            ...
            if (pKFi != mpCurrentKF) {
                ...
                g2o::Sim3 g2oSic(
                    Converter::toMatrix3d(Ric), Converter::toVector3d(tic),
                    1.0);
        ...
        for (KeyFrameAndPose::iterator mit = CorrectedSim3.begin(),
            ...
            if (bImuInit) {
                ...
                pKFi->SetVelocity(
                    Converter::toCvMat(Rcor) * pKFi->GetVelocity());
            }
    ...
    if (!pLoopMap->isImuInitialized() ||
        ...
        mpThreadGBA = new thread(
            &LoopClosing::RunGlobalBundleAdjustment, this, pLoopMap,
            mpCurrentKF->mnId);
    }
...
void LoopClosing::MergeLocal() {
    Verbose::PrintMess(
        "MERGE-VISUAL: Merge Visual detected!!!!", Verbose::VERBOSITY_NORMAL);
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Check Full Bundle Adjustment", Verbose::VERBOSITY_DEBUG);
    // If a Global Bundle Adjustment is running, abort it
    if (isRunningGBA()) {
        unique_lock<mutex> lock(mMutexGBA);
        mbStopGBA = true;
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Request Stop Local Mapping", Verbose::VERBOSITY_DEBUG);
    mpLocalMapper->RequestStop();
    // Wait until Local Mapping has effectively stopped
    while (!mpLocalMapper->isStopped()) {
        usleep(1000);
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Local Map stopped", Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Initially there are " +
            ...
            to_string(pCurrentMap->MapPointsInMap()) +
            " MPs in the active map ",
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "MERGE-VISUAL: Initially there are " +
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Initial number of KFs in local window from active "
        "map: " +
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Last number of KFs in local window from the active "
        "map: " +
            to_string(spLocalWindowKFs.size()),
        Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess(
        ...
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "MERGE-VISUAL: Number of MPs in the active map: " +
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Last number of KFs in the localwindow from matched "
        "map: " +
            to_string(spMergeConnectedKFs.size()),
        Verbose::VERBOSITY_DEBUG);
    ...
    std::copy(
        spMapPointMerge.begin(), spMapPointMerge.end(),
    ...
    for (KeyFrame *pKFi : spLocalWindowKFs) {
        if (!pKFi || pKFi->isBad()) {
            Verbose::PrintMess(
                "Bad KF in correction", Verbose::VERBOSITY_DEBUG);
            continue;
        }
        ...
        // cout << "angle: " <<
        // 180*LogSO3(mg2oMergeScw.rotation().toRotationMatrix())/3.14 << endl;
    ...
    Verbose::PrintMess(
        ...
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "MERGE-VISUAL: LOCAL MAPPING number of KFs: " +
    ...
    bool bStop = false;
    Verbose::PrintMess(
        "MERGE-VISUAL: Start local BA ", Verbose::VERBOSITY_DEBUG);
    vpLocalCurrentWindowKFs.clear();
    vpMergeConnectedKFs.clear();
    std::copy(
        spLocalWindowKFs.begin(), spLocalWindowKFs.end(),
    ...
    // return;
    Verbose::PrintMess(
        "MERGE-VISUAL: Finish the LBA", Verbose::VERBOSITY_DEBUG);
    ...
    if (vpCurrentMapKFs.size() == 0) {
        ...
        Verbose::PrintMess(
            "MERGE-VISUAL: Calculate the new position of the elements outside "
            "of the window",
            Verbose::VERBOSITY_DEBUG);
        // Apply the transformation
        {
            if (mpTracker->mSensor == System::MONOCULAR) {
                ...
                for (KeyFrame *pKFi : vpCurrentMapKFs) {
                    ...
                    cv::Mat tiw = Tiw.rowRange(0, 3).col(3);
                    g2o::Sim3 g2oSiw(
                        Converter::toMatrix3d(Riw), Converter::toVector3d(tiw),
                        1.0);
                    // Pose without correction
                    vNonCorrectedSim3[pKFi] = g2oSiw;
                    ...
                    if (pCurrentMap->isImuInitialized()) {
                        Eigen::Matrix3d Rcor =
                            eigR.transpose() * vNonCorrectedSim3[pKFi]
                                                   .rotation()
                                                   .toRotationMatrix();
                        pKFi->SetVelocity(
                            Converter::toCvMat(Rcor) *
                            pKFi->GetVelocity()); // TODO: should add here scale
                                                  // s
                    }
                }
...

        {
            ...
            Verbose::PrintMess(
                "MERGE-VISUAL: There are " +
                    to_string(pMergeMap->KeyFramesInMap()) + " KFs in the map",
                Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess(
                "MERGE-VISUAL: It will be inserted " +
        ...
        Verbose::PrintMess(
            "MERGE-VISUAL: Optimaze the essential graph",
            Verbose::VERBOSITY_DEBUG);
    }
    ...
    Verbose::PrintMess(
        "MERGE-VISUAL: Finally there are " +
            to_string(pMergeMap->KeyFramesInMap()) + "KFs and " +
    ...
    if (bRelaunchBA &&
        ...
        mbStopGBA = false;
        mpThreadGBA = new thread(
            &LoopClosing::RunGlobalBundleAdjustment, this, pMergeMap,
            mpCurrentKF->mnId);
    }
...
void LoopClosing::printReprojectionError(
    set<KeyFrame *> &spLocalWindowKFs, KeyFrame *mpCurrentKF, string &name) {
    ...
    for (KeyFrame *pKFi : spLocalWindowKFs) {
        ...
        for (int j = 0; j < vpMPs.size(); ++j) {
            ...
            float u, v;
            bool bIsInImage =
                pKFi->ProjectPointUnDistort(pMPij, reproj_p, u, v);
            if (bIsInImage) {
                // cout << "Reproj in the image" << endl;
                cv::circle(
                    img_i, point_img.pt, 1 /*point_img.octave*/,
                    cv::Scalar(0, 255, 0));
                ...
                cv::circle(
                    img_i, point_img.pt, point_img.octave,
                    cv::Scalar(0, 0, 255));
            }
        }
...
void LoopClosing::CheckObservations(
    set<KeyFrame *> &spKFsMap1, set<KeyFrame *> &spKFsMap2) {
    cout << "----------------------" << endl;
...
void LoopClosing::SearchAndFuse(
    const KeyFrameAndPose &CorrectedPosesMap, vector<MapPoint *> &vpMapPoints) {
...
void LoopClosing::SearchAndFuse(
    const vector<KeyFrame *> &vConectedKFs, vector<MapPoint *> &vpMapPoints) {
    ORBmatcher matcher(0.8);
...
void LoopClosing::RunGlobalBundleAdjustment(
    Map *pActiveMap, unsigned long nLoopKF) {
    Verbose::PrintMess(
            ...
            while (!lpKFtoCheck.empty()) {
                ...
                if (pKF->bImu) {
                    ...
                    if (pKF->mVwbGBA.empty())
                        Verbose::PrintMess(
                            "pKF->mVwbGBA is empty", Verbose::VERBOSITY_NORMAL);
