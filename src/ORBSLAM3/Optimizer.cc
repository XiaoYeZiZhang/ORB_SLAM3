...
#include <mutex>
#include "include/ORBSLAM3/OptimizableTypes.h"
...
// void Optimizer::GlobalBundleAdjustemnt_Superpoint(
//    Map *pMap, int nIterations, bool *pbStopFlag, const unsigned long nLoopKF,
//    const bool bRobust) {
//    vector<KeyFrame *> vpKFs = pMap->GetAllKeyFrames();
//    vector<MapPoint *> vpMP = pMap->GetAllMapPoints();
//    BundleAdjustment_SuperPoint(
//        vpKFs, vpMP, nIterations, pbStopFlag, nLoopKF, bRobust);
//}
...
void Optimizer::BundleAdjustment(
    ...
    Verbose::PrintMess(
        "BA: End of the optimization", Verbose::VERBOSITY_NORMAL);
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (nLoopKF == pMap->GetOriginKF()->mnId) {
            ...
            /*if(!vSE3->fixed())
            {
                ...
                for(int r=0; r<6; ++r)
                {
                    for(int c=0; c<6; ++c)
                    {
                        ...
                        pKF->mHessianPose.at<double>(r, c) = vSE3->hessian(r,
            c);
                    }
                    //cout << endl;
                }
            ...
            if (dist > 1) {
                ...
                Verbose::PrintMess(
                    "GBA: KF " + to_string(pKF->mnId) + " had been moved " +
                        to_string(dist) + " meters",
                    Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess(
                    "--Number of observations: " + to_string(numMonoOptPoints) +
                        " in mono and " + to_string(numStereoOptPoints) +
                        " in stereo",
                    Verbose::VERBOSITY_DEBUG);
                Verbose::PrintMess(
...
// void Optimizer::BundleAdjustment_SuperPoint(
//    const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
//    int nIterations, bool *pbStopFlag, const unsigned long nLoopKF,
//    const bool bRobust) {
//    vector<bool> vbNotIncludedMP;
//    vbNotIncludedMP.resize(vpMP.size());
//
//    Map *pMap_superpoint = vpKFs[0]->GetMap_SuperPoint();
//
//    g2o::SparseOptimizer optimizer;
//    g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
//
...
//        vSE3->setEstimate(Converter::toSE3Quat(pKF->GetPose()));
//        vSE3->setId(pKF->mnId);
//        // set all the keyframe fixed
//        // TODO(zhangye): fix this, set all keyframe fixed
//        vSE3->setFixed(pKF->mnId != pMap_superpoint->GetMaxKFid());
//        optimizer.addVertex(vSE3);
//        if (pKF->mnId > maxKFid)
//            maxKFid = pKF->mnId;
//        // cout << "KF id: " << pKF->mnId << endl;
...
//
//            if (leftIndex != -1) {
//                const cv::KeyPoint &kpUn_superpoint =
//                pKF->mvKeysUn_superpoint[leftIndex];
//
//                Eigen::Matrix<double, 2, 1> obs_superpoint;
//                obs_superpoint << kpUn_superpoint.pt.x, kpUn_superpoint.pt.y;
//
//                ORB_SLAM3::EdgeSE3ProjectXYZ *e =
//                    new ORB_SLAM3::EdgeSE3ProjectXYZ();
//
...
//                    1, dynamic_cast<g2o::OptimizableGraph::Vertex *>(
//                           optimizer.vertex(pKF->mnId)));
//                e->setMeasurement(obs_superpoint);
//                const float &invSigma2 =
//                pKF->mvInvLevelSigma2_suerpoint[kpUn_superpoint.octave];
//                e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
//
//                if (bRobust) {
//                    g2o::RobustKernelHuber *rk = new g2o::RobustKernelHuber;
...
//
//        g2o::SE3Quat SE3quat = vSE3->estimate();
//        if (nLoopKF == pMap_superpoint->GetOriginKF()->mnId) {
//            pKF->SetPose(Converter::toCvMat(SE3quat));
//        } else {
//            /*if(!vSE3->fixed())
//            {
...
void Optimizer::FullInertialBA(
    Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId,
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            ...
            if (pKFi->bImu && pKFi->mPrevKF->bImu) {
                ...
                if (!bInit) {
                    VG1 = optimizer.vertex(
                        maxKFid + 3 * (pKFi->mPrevKF->mnId) + 2);
                    VA1 = optimizer.vertex(
                        maxKFid + 3 * (pKFi->mPrevKF->mnId) + 3);
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (nLoopId == 0) {
            cv::Mat Tcw = Converter::toCvSE3(
                VP->estimate().Rcw[0], VP->estimate().tcw[0]);
            pKFi->SetPose(Tcw);
        } else {
...
void Optimizer::LocalBundleAdjustment_Superpoint(Atlas *atlas_superpoint) {
    ...
    VLOG(0) << "SuperPoint LBA: There are " +
                   std::to_string(lLocalKeyFrames.size())
            << " KFs and " << lLocalMapPoints.size() << " MPs to optimize. "
            << num_fixedKF << " KFs are fixed.";
    ...
    for (std::vector<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
        ...
        for (map<KeyFrame *, tuple<int, int>>::const_iterator
             ...
             mit != mend; mit++) {
            KeyFrame *pKFi = mit->first;
            if (!pKFi->isBad() && pKFi->GetMap_SuperPoint() == currentMap) {
                const int cam0Index = get<0>(mit->second);
                // Monocular observation of Camera 0
                if (cam0Index != -1) {
                    const cv::KeyPoint &kpUn_superpoint =
                        pKFi->mvKeysUn_superpoint[cam0Index];
                    Eigen::Matrix<double, 2, 1> obs;
                    obs << kpUn_superpoint.pt.x, kpUn_superpoint.pt.y;
                    ...
                    e->setMeasurement(obs);
                    const float &invSigma2 =
                        pKFi->mvInvLevelSigma2_suerpoint[kpUn_superpoint
                                                             .octave];
                    e->setInformation(Eigen::Matrix2d::Identity() * invSigma2);
    ...
    VLOG(0) << "SuperPoint LBA: Fist optimization has " << nMonoBadObs
            << " monocular and " << nStereoBadObs
            << " stereo bad observations.";

    // Optimize again without the outliers
    VLOG(0) << "Superpoint LBA: second optimization";
    ...
    // Get Map Mutex
    unique_lock<mutex> lock(currentMap->mMutexMapUpdate);

    if (!vToErase.empty()) {
        VLOG(0) << "Superpoint LBA: There are " << vToErase.size()
                << " observations whose will"
...
void Optimizer::LocalBundleAdjustment(
    KeyFrame *pKF, bool *pbStopFlag, vector<KeyFrame *> &vpNonEnoughOptKFs) {
    ...
    // Verbose::PrintMess("LM-LBA: There are " +
    // to_string(lLocalKeyFrames.size()) + " KFs and " +
    // to_string(lLocalMapPoints.size()) + " MPs to optimize. " +
    ...
    Verbose::PrintMess(
        ...
        Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
        "LM-LBA: local MPs: " + to_string(lLocalMapPoints.size()),
        Verbose::VERBOSITY_DEBUG);
    ...
    if (bDoMore) {
        ...
        Verbose::PrintMess(
            "LM-LBA: First optimization has " + to_string(nMonoBadObs) +
                " monocular and " + to_string(nStereoBadObs) +
                " stereo bad observations",
            Verbose::VERBOSITY_DEBUG);
    ...
    if (!vToErase.empty()) {
        ...
        // cout << "LM-LBA: There are " << vToErase.size() << " observations
        // whose will be deleted from the map" << endl;
        for (size_t i = 0; i < vToErase.size(); i++) {
            KeyFrame *pKFi = vToErase[i].first;
            MapPoint *pMPi = vToErase[i].second;
            pKFi->EraseMapPointMatch(pMPi);
...
void Optimizer::LocalBundleAdjustment(
    KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int &num_fixedKF) {
    // cout << "LBA" << endl;
    // Local KeyFrames: First Breath Search from Current Keyframe
    list<KeyFrame *> lLocalKeyFrames;
        ...
        for (map<KeyFrame *, tuple<int, int>>::const_iterator
            ...
            if (!pKFi->isBad() && pKFi->GetMap() == pCurrentMap) {
                ...
                if (leftIndex != -1 &&
                    ...
                    leftIndex != -1 && pKFi->mvuRight[get<0>(mit->second)] >=
    ...
    if (!vToErase.empty()) {
        ...
        if (bRedrawError) {
            for (KeyFrame *pKFi : lLocalKeyFrames) {
                ...
                mspInitialConnectedKFs[pKFi] =
                    pKFi->GetConnectedKeyFrames().size();
        ...
        if (bRedrawError) {
            ofstream f_lba;
            f_lba.open(
                "test_LBA/LBA_failure_KF" + to_string(pKF->mnId) + ".txt");
            f_lba << "# KF id, Initial Num CovKFs, Final Num CovKFs, Initial "
                     "Num MPs, Fimal Num MPs"
                  << endl;
            f_lba << fixed;
    ...
    for (list<KeyFrame *>::iterator lit = lLocalKeyFrames.begin(),
        ...
        if (dist > 1.0) {
            bShowStats = true;
            Verbose::PrintMess(
                "LM-LBA: Too much distance in KF " + to_string(pKFi->mnId) +
                    ", " + to_string(dist) + " meters. Current KF " +
                    to_string(pKF->mnId),
                Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess(
                "LM-LBA: Number of connections between the KFs " +
            ...
            Verbose::PrintMess(
                "LM-LBA: Good observations in mono " + to_string(numMonoMP) +
                    " and stereo " + to_string(numStereoMP),
                Verbose::VERBOSITY_DEBUG);
            Verbose::PrintMess(
                "LM-LBA: Bad observations in mono " + to_string(numBadMonoMP) +
    ...
    if (bRedrawError) {
        ...
        // pMap->printReprojectionError(lLocalKeyFrames, pKF, name,
        // folder_name);
...
void Optimizer::OptimizeEssentialGraph(
    ...
    for (size_t i = 0, iend = vpKFs.size(); i < iend; i++) {
        ...
        /*if(count_kf<3)
            cout << "EG: kf with " << count_kf << " edges!!    ID: " <<
           pKF->mnId << endl;*/
    }
...
void Optimizer::OptimizeEssentialGraph6DoF(
    ...
    vector<g2o::SE3Quat, Eigen::aligned_allocator<g2o::SE3Quat>> vScw(
        nMaxKFid + 1);
    ...
    Verbose::PrintMess(
        "Opt_Essential: vpFixedCorrectedKFs loaded", Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess(
        "Opt_Essential: vpNonFixedKFs loaded", Verbose::VERBOSITY_DEBUG);
    ...
    Verbose::PrintMess(
        "Opt_Essential: List of KF loaded", Verbose::VERBOSITY_DEBUG);
    ...
    for (KeyFrame *pKFi : vpKFs) {
        ...
        if (pParentKFi && spKFs.find(pParentKFi) != spKFs.end()) {
            ...
            else
            {
                ...
                Sjw = g2o::Sim3(Converter::toMatrix3d(Tjw.rowRange(0,
            3).colRange(0, 3)), Converter::toVector3d(Tjw.rowRange(0,
            3).col(3)),1.0);
            }*/
        ...
        for (set<KeyFrame *>::const_iterator sit = sLoopEdges.begin(),
            ...
            if (spKFs.find(pLKF) != spKFs.end() && pLKF->mnId < pKFi->mnId) {
                ...
                else
                {
                    ...
                    Slw = g2o::Sim3(Converter::toMatrix3d(Tlw.rowRange(0,
                3).colRange(0, 3)), Converter::toVector3d(Tlw.rowRange(0,
                3).col(3)),1.0);
                }*/
        ...
        if (num_connections == 0) {
            Verbose::PrintMess(
                "Opt_Essential: KF " + to_string(pKFi->mnId) +
                    " has 0 connections",
                Verbose::VERBOSITY_DEBUG);
        }
    }
    ...
    Verbose::PrintMess(
        "Opt_Essential: Finish the optimization", Verbose::VERBOSITY_DEBUG);

    unique_lock<mutex> lock(pMap->mMutexMapUpdate);

    Verbose::PrintMess(
        "Opt_Essential: Apply the new pose to the KFs",
        Verbose::VERBOSITY_DEBUG);
    // SE3 Pose Recovering. Sim3:[sR t;0 1] -> SE3:[R t/s;0 1]
    for (KeyFrame *pKFi : vpNonFixedKFs) {
        if (pKFi->isBad())
        ...
        connected " << pKFi->GetVectorCovisibleKeyFrames().size() << " KFs" <<
        endl;
            }
            ...
            for(int j=0; j<vpMapPointsKFi.size(); ++j)
            {
                ...
                cv::circle(imLeft, pKFi->mvKeys[j].pt, 2, cv::Scalar(0, 255,
        0)); cv::putText(imLeft, strNumOBs, pKFi->mvKeys[j].pt,
        CV_FONT_HERSHEY_DUPLEX, 1, cv::Scalar(255, 0, 0));
            }

            string namefile = "./test_OptEssent/Essent_" +
        to_string(pCurKF->mnId) + "_KF" + to_string(pKFi->mnId) +"_D" +
    ...
    Verbose::PrintMess(
        "Opt_Essential: Apply the new pose to the MPs",
        Verbose::VERBOSITY_DEBUG);
    ...
    for (MapPoint *pMPi : vpNonCorrectedMPs) {
        ...
        // cout << "Opt_Essential: Loaded the corrected position in the MP
        // object" << endl;
    ...
    Verbose::PrintMess(
        "Opt_Essential: End of the optimization", Verbose::VERBOSITY_DEBUG);
}
...
void Optimizer::OptimizeEssentialGraph(
    ...
    for (MapPoint *pMPi : vpNonCorrectedMPs) {
        ...
        Verbose::PrintMess(
            "Opt_Essential: MP id " + to_string(pMPi->mnId),
            Verbose::VERBOSITY_DEBUG);
        /*int nIDr;
        if(pMPi->mnCorrectedByKF==pCurKF->mnId)
        {
...
void Optimizer::OptimizeEssentialGraph(
    ...
    Verbose::PrintMess(
        "edges pose graph: " + to_string(count_edges[0]) + ", " +
...
int Optimizer::OptimizeSim3(
    ...
    Verbose::PrintMess(
        "Sim3: There are " + to_string(nCorrespondences) + " matches, " +
            ...
            " are in the connected KFs. There are " +
            to_string(nMatchWithoutMP) +
...
int Optimizer::OptimizeSim3(
    ...
    const cv::Mat t1w = pKF1->GetTranslation();
    Verbose::PrintMess(
        "Extracted rotation and traslation from the first KF ",
        Verbose::VERBOSITY_DEBUG);
    const cv::Mat R2w = pKF2->GetRotation();
    const cv::Mat t2w = pKF2->GetTranslation();
    Verbose::PrintMess(
        "Extracted rotation and traslation from the second KF ",
        Verbose::VERBOSITY_DEBUG);
...
void Optimizer::LocalInertialBA(
    ...
    Verbose::PrintMess(
        "LIBA bNonFixed?: " + to_string(bNonFixed), Verbose::VERBOSITY_DEBUG);
    Verbose::PrintMess(
...
Eigen::MatrixXd Optimizer::Condition(
    const Eigen::MatrixXd &H, const int &start, const int &end) {
...
Eigen::MatrixXd Optimizer::Sparsify(
    const Eigen::MatrixXd &H, const int &start1, const int &end1,
...
void Optimizer::InertialOptimization(
    Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg,
    ...
    for (size_t i = 0; i < vpKFs.size(); i++) {
        ...
        if (pKFi->mPrevKF && pKFi->mnId <= maxKFid) {
            ...
            ei->setVertex(
                5, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VV2));
            ei->setVertex(
                6, dynamic_cast<g2o::OptimizableGraph::Vertex *>(VGDir));
...
void Optimizer::InertialOptimization(
    Map *pMap, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG,
    float priorA) {
    int its = 200; // Check number of iterations
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const vector<KeyFrame *> vpKFs = pMap->GetAllKeyFrames();
...
void Optimizer::InertialOptimization(
    vector<KeyFrame *> vpKFs, Eigen::Vector3d &bg, Eigen::Vector3d &ba,
...
void Optimizer::InertialOptimization(
    Map *pMap, Eigen::Matrix3d &Rwg, double &scale) {
    int its = 10;
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const vector<KeyFrame *> vpKFs = pMap->GetAllKeyFrames();
...
void Optimizer::MergeBundleAdjustmentVisual(
    KeyFrame *pCurrentKF, vector<KeyFrame *> vpWeldingKFs,
...
void Optimizer::LocalBundleAdjustment(
    KeyFrame *pMainKF, vector<KeyFrame *> vpAdjustKF,
    ...
    for (map<MapPoint *, int>::iterator it = mpObsMPs.begin();
        ...
        /*if(numObs < 5)
        {
            cout << "LBA: MP " << pMPi->mnId << " has " << numObs << "
        observations" << endl;
        }*/
    }
    ...
    if (bDoMore) {
        ...
        Verbose::PrintMess(
            "LBA: First optimization, there are " + to_string(badMonoMP) +
    ...
    Verbose::PrintMess(
        "LBA: Second optimization, there are " + to_string(badMonoMP) +
    ...
    if (!vToErase.empty()) {
        ...
        Verbose::PrintMess(
            "LBA: " + to_string(spErasedMPs.size()) +
            ...
            if (pMPi->isBad()) {
                Verbose::PrintMess(
                    "LBA: MP " + to_string(pMPi->mnId) +
                        " has lost almost all the observations, its origin map "
                        "is " +
        ...
        for (KeyFrame *pKFi : spErasedKFs) {
            ...
            Verbose::PrintMess(
                "LBA: Initially KF " + to_string(pKFi->mnId) + " had " +
                    to_string(num_init_MPs) + ", at the end has " +
                    to_string(num_MPs),
                Verbose::VERBOSITY_DEBUG);
        }
    }
    ...
    //                    }
    //                    int index =
    //                        get<0>(vpMonoMPsOpt[i]->GetIndexInKeyFrame(pKFi));
    //                    if (index < 0) {
    //                        // cout << "LBA ERROR: KF has a monocular
    //                        observation
    //                        // which is not recognized by the MP" << endl;
    //                        cout <<
    //                        // "LBA: KF " << pKFi->mnId << " and MP " <<
    //                        // vpMonoMPsOpt[i]->mnId << " with index " <<
    //                        endl; continue;
    //                    }
    //
    //                    // string strNumOBs =
    ...
    //                    }
    //                    int index =
    //                        get<0>(vpStereoMPsOpt[i]->GetIndexInKeyFrame(pKFi));
    //                    if (index < 0) {
    //                        // cout << "LBA: KF has a stereo observation which
    //                        is
    //                        // not recognized by the MP" << endl; cout <<
    //                        "LBA: KF "
    //                        // << pKFi->mnId << " and MP " <<
    //                        // vpStereoMPsOpt[i]->mnId
    //                        // << endl;
    //                        continue;
    //                    }
    //
    ...
    //                    }
    //                    int index =
    //                        get<0>(vpMonoMPsBad[i]->GetIndexInKeyFrame(pKFi));
    //                    if (index < 0) {
    //                        // cout << "LBA ERROR: KF has a monocular
    //                        observation
    //                        // which is not recognized by the MP" << endl;
    ...
    //                    }
    //                    int index =
    //                        get<0>(vpStereoMPsBad[i]->GetIndexInKeyFrame(pKFi));
    //                    if (index < 0) {
    //                        // cout << "LBA: KF has a stereo observation which
    //                        is
    //                        // not recognized by the MP" << endl; cout <<
    ...
    //                }
    //
    //                string namefile = "./test_LBA/LBA_KF" +
    //                to_string(pKFi->mnId) +
    //                                  "_" +
    //                                  to_string(numPointsMono +
    //                                  numPointsStereo) +
    //                                  "_D" + to_string(dist) + ".png";
    //                cv::imwrite(namefile, imLeft);
    //
    //                Verbose::PrintMess(
    //                    "--LBA in KF " + to_string(pKFi->mnId),
    //                    Verbose::VERBOSITY_DEBUG);
    //                Verbose::PrintMess(
    //                    "--Distance: " + to_string(dist) + " meters",
    //                    Verbose::VERBOSITY_DEBUG);
    //                Verbose::PrintMess(
    //                    "--Number of observations: " +
    //                    to_string(numMonoOptPoints) +
    //                        " in mono and " + to_string(numStereoOptPoints) +
    //                        " in stereo",
    //                    Verbose::VERBOSITY_DEBUG);
    //                Verbose::PrintMess(
    ...
    //                    Verbose::VERBOSITY_DEBUG);
    //                Verbose::PrintMess(
    //                    "--To much distance correction in LBA: It has " +
...
void Optimizer::MergeInertialBA(
    KeyFrame *pCurrKF, KeyFrame *pMergeKF, bool *pbStopFlag, Map *pMap,
    ...
    Verbose::PrintMess(
        "end inserting inertial edges", Verbose::VERBOSITY_NORMAL);
    ...
    for (list<MapPoint *>::iterator lit = lLocalMapPoints.begin(),
        ...
        for (map<KeyFrame *, tuple<int, int>>::const_iterator
            ...
            if (pKFi->mnId > maxKFid) {
                Verbose::PrintMess(
                    "ID greater than current KF is", Verbose::VERBOSITY_NORMAL);
                continue;
            }
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(
    ...
    pFrame->SetImuPoseVelocity(
        Converter::toCvMat(VP->estimate().Rwb),
