...
void FrameDrawer::DrawFrame(bool bOldFeatures) {
    ...
    if (state == Tracking::NOT_INITIALIZED) {
        for (unsigned int i = 0; i < vMatches.size(); i++) {
            if (vMatches[i] >= 0) {
                cv::line(
                    im, vIniKeys[i].pt, vCurrentKeys[vMatches[i]].pt,
                    cv::Scalar(0, 255, 0));
            }
        }
    ...
    } else if (state == Tracking::OK && bOldFeatures) // TRACKING
    {
        ...
        //cout << "+Point CV " << point3d_proy.x << ", " << point3d_proy.y <<
        endl;
        ...
        for (int i = 0; i < n; ++i) {
            cv::Point2f point3d_proy;
            float u, v;
            currentFrame.ProjectPointDistort(
                vpOutlierMPs[i], point3d_proy, u, v);
        ...
        for (int i = 0; i < n; i++) {
            if (vbVO[i + Nleft] || vbMap[i + Nleft]) {
                ...
                if (vbMap[i + Nleft]) {
                    cv::rectangle(im, pt1, pt2, cv::Scalar(0, 255, 0));
                    cv::circle(
                        im, mvCurrentKeysRight[i].pt, 2, cv::Scalar(0, 255, 0),
                        -1);
                    mnTracked++;
                } else // This is match to a "visual odometry" MapPoint created
                       // in the last frame
                {
                    cv::rectangle(im, pt1, pt2, cv::Scalar(255, 0, 0));
                    cv::circle(
                        im, mvCurrentKeysRight[i].pt, 2, cv::Scalar(255, 0, 0),
                        -1);
                    mnTrackedVO++;
                }
            }
...
void FrameDrawer::DrawTextInfo(cv::Mat &im, int nState, cv::Mat &imText) {
    ...
    cv::putText(
        imText, s.str(), cv::Point(5, imText.rows - 5), cv::FONT_HERSHEY_PLAIN,
