...
#include "include/ORBSLAM3/ORBextractor.h"
...
Frame::Frame(const Frame &frame)
    : mpcpi(frame.mpcpi), mpORBvocabulary(frame.mpORBvocabulary),
      ...
      mTrl(frame.mTrl.clone()), mTimeStereoMatch(frame.mTimeStereoMatch),
      mTimeORB_Ext(frame.mTimeORB_Ext), imgLeft(frame.imgLeft.clone()) {
...
Frame::Frame(
    const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor,
    ...
    : mpcpi(NULL), mpORBvocabulary(voc), mpORBextractorLeft(extractor),
      ...
      mpImuPreintegratedFrame(NULL),
      mpReferenceKF(static_cast<KeyFrame *>(NULL)), mbImuPreintegrated(false),
      mpCamera(pCamera), mpCamera2(nullptr), mTimeStereoMatch(0),
      mTimeORB_Ext(0) {
    // Frame ID
    mnId = nNextId++;
...
void Frame::ExtractORB(
    int flag, const cv::Mat &im, const int x0, const int x1) {
...
void Frame::SetImuPoseVelocity(
    const cv::Mat &Rwb, const cv::Mat &twb, const cv::Mat &Vwb) {
    mVw = Vwb.clone();
    cv::Mat Rbw = Rwb.t();
    cv::Mat tbw = -Rbw * twb;
...
void Frame::ComputeImageBounds(const cv::Mat &imLeft) {
    if (mDistCoef.at<float>(0) != 0.0) {
        ...
        mat = mat.reshape(2);
        cv::undistortPoints(
            mat, mat, static_cast<Pinhole *>(mpCamera)->toK(), mDistCoef,
            cv::Mat(), mK);
        mat = mat.reshape(1);
...
void Frame::ComputeStereoMatches() {
    ...
    for (int iL = 0; iL < N; iL++) {
        ...
        if (bestDist < thOrbDist) {
            ...
            // *cv::Mat::ones(IL.rows,IL.cols,CV_32F);
...
bool Frame::isInFrustumChecks(
    MapPoint *pMP, float viewingCosLimit, bool bRight) {
    // 3D in absolute coordinates
    cv::Mat P = pMP->GetWorldPos();
