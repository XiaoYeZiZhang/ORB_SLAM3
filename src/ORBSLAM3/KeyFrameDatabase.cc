...
#include "include/ORBSLAM3/KeyFrameDatabase.h"
...
vector<KeyFrame *>
KeyFrameDatabase::DetectLoopCandidates(KeyFrame *pKF, float minScore) {
...
void KeyFrameDatabase::DetectCandidates(
    KeyFrame *pKF, float minScore, vector<KeyFrame *> &vpLoopCand,
...
void KeyFrameDatabase::DetectBestCandidates(
    KeyFrame *pKF, vector<KeyFrame *> &vpLoopCand,
    ...
    // Search all keyframes that share a word with current frame
    {
        ...
        for (DBoW2::BowVector::const_iterator vit = pKF->mBowVec.begin(),
            ...
            for (list<KeyFrame *>::iterator lit = lKFs.begin(),
                ...
                pKFi->mnPlaceRecognitionWords++;
                /*if(spInsertedKFsSharing.find(pKFi) ==
                spInsertedKFsSharing.end())
                {
                    lKFsSharingWords.push_back(pKFi);
...
void KeyFrameDatabase::PreSave() {
    ...
    for (int i = 0, numEl = mvInvertedFile.size(); i < numEl; ++i) {
        for (std::list<KeyFrame *>::const_iterator
                 it = mvInvertedFile[i].begin(),
