...
TwoViewReconstruction::TwoViewReconstruction(
    cv::Mat &K, float sigma, int iterations) {
    mK = K.clone();
...
bool TwoViewReconstruction::Reconstruct(
    ...
    thread threadH(
        &TwoViewReconstruction::FindHomography, this, ref(vbMatchesInliersH),
        ref(SH), ref(H));
    thread threadF(
        &TwoViewReconstruction::FindFundamental, this, ref(vbMatchesInliersF),
        ref(SF), ref(F));
    ...
    if (RH > 0.50) // if(RH>0.40)
    {
        ...
        return ReconstructH(
            vbMatchesInliersH, H, mK, R21, t21, vP3D, vbTriangulated,
            minParallax, 50);
    } else // if(pF_HF>0.6)
    {
        // cout << "Initialization from Fundamental" << endl;
        return ReconstructF(
            vbMatchesInliersF, F, mK, R21, t21, vP3D, vbTriangulated,
            minParallax, 50);
    }
}
...
cv::Mat TwoViewReconstruction::ComputeH21(
...
cv::Mat TwoViewReconstruction::ComputeF21(
    const vector<cv::Point2f> &vP1, const vector<cv::Point2f> &vP2) {
    const int N = vP1.size();
...
float TwoViewReconstruction::CheckHomography(
    const cv::Mat &H21, const cv::Mat &H12, vector<bool> &vbMatchesInliers,
    float sigma) {
    const int N = mvMatches12.size();
...
bool TwoViewReconstruction::ReconstructF(
    ...
    int nGood1 = CheckRT(
        R1, t1, mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers, K, vP3D1,
        4.0 * mSigma2, vbTriangulated1, parallax1);
    int nGood2 = CheckRT(
        R2, t1, mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers, K, vP3D2,
        4.0 * mSigma2, vbTriangulated2, parallax2);
    int nGood3 = CheckRT(
        R1, t2, mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers, K, vP3D3,
        4.0 * mSigma2, vbTriangulated3, parallax3);
    int nGood4 = CheckRT(
        R2, t2, mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers, K, vP3D4,
...
bool TwoViewReconstruction::ReconstructH(
    ...
    for (size_t i = 0; i < 8; i++) {
        ...
        int nGood = CheckRT(
            vR[i], vt[i], mvKeys1, mvKeys2, mvMatches12, vbMatchesInliers, K,
...
void TwoViewReconstruction::Triangulate(
    const cv::KeyPoint &kp1, const cv::KeyPoint &kp2, const cv::Mat &P1,
...
void TwoViewReconstruction::Normalize(
    const vector<cv::KeyPoint> &vKeys, vector<cv::Point2f> &vNormalizedPoints,
    cv::Mat &T) {
    float meanX = 0;
    float meanY = 0;
