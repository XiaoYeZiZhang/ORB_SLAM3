...
cv::Mat KannalaBrandt8::projectJac(const cv::Point3f &p3D) {
    ...
    Jac.at<float>(1, 0) =
        mvParameters[1] * (fd * p3D.z * p3D.y * p3D.x / (r2 * (r2 + z2)) -
                           f * p3D.y * p3D.x / r3);

    Jac.at<float>(0, 1) =
        mvParameters[0] * (fd * p3D.z * p3D.y * p3D.x / (r2 * (r2 + z2)) -
                           f * p3D.y * p3D.x / r3);
    Jac.at<float>(1, 1) =
...
bool KannalaBrandt8::ReconstructWithTwoViews(
    ...
    cv::Mat D =
        (cv::Mat_<float>(4, 1) << mvParameters[4], mvParameters[5],
...
cv::Mat KannalaBrandt8::toK() {
    cv::Mat K =
        (cv::Mat_<float>(3, 3) << mvParameters[0], 0.f, mvParameters[2], 0.f,
...
bool KannalaBrandt8::matchAndtriangulate(
    ...
    cv::Mat &Tcw1, cv::Mat &Tcw2, const float sigmaLevel1,
...
float KannalaBrandt8::TriangulateMatches(
    ...
    cv::Mat x3D;
    cv::Mat Tcw1 =
        (cv::Mat_<float>(3, 4) << 1.f, 0.f, 0.f, 0.f, 0.f, 1.f, 0.f, 0.f, 0.f,
         0.f, 1.f, 0.f);
    cv::Mat Tcw2;
    cv::Mat R21 = R12.t();
    cv::Mat t21 = -R21 * t12;
...
void KannalaBrandt8::Triangulate(
    const cv::Point2f &p1, const cv::Point2f &p2, const cv::Mat &Tcw1,
