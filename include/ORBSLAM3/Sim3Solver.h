...
public:
    ...
    cv::Mat iterate(
        int nIterations, bool &bNoMore, std::vector<bool> &vbInliers,
        int &nInliers);
    cv::Mat iterate(
...
protected:
    ...
    void Project(
        const std::vector<cv::Mat> &vP3Dw, std::vector<cv::Mat> &vP2D,
        cv::Mat Tcw, GeometricCamera *pCamera);
    void FromCameraToImage(
        const std::vector<cv::Mat> &vP3Dc, std::vector<cv::Mat> &vP2D,
