...
public:
    ...
    void static FullInertialBA(
        Map *pMap, int its, const bool bFixLocal = false,
    ...
    void static LocalBundleAdjustment_Superpoint(Atlas *atlas_superpoint);
    void static LocalBundleAdjustment(
    ...
    static int OptimizeSim3(
        KeyFrame *pKF1, KeyFrame *pKF2, std::vector<MapPoint *> &vpMatches1,
    ...
    void static LocalInertialBA(
        KeyFrame *pKF, bool *pbStopFlag, Map *pMap, bool bLarge = false,
        bool bRecInit = false);
    ...
    static Eigen::MatrixXd
    Condition(const Eigen::MatrixXd &H, const int &start, const int &end);
    // Remove link between element 1 and 2. Given elements 1,2 and 3 must define
    // the whole matrix.
    static Eigen::MatrixXd Sparsify(
    ...
    void static InertialOptimization(
        ...
        float priorA = 1e6);
    void static InertialOptimization(
        Map *pMap, Eigen::Vector3d &bg, Eigen::Vector3d &ba, float priorG = 1e2,
        float priorA = 1e6);
    void static InertialOptimization(
